...
- **在改动完成且 `npm test` 通过后**：由 Agent **直接 `git commit` 并 `git push`** 到远程**默认主干**（本仓库为 **`main`**；若你方远程主干名为 `master`，则推送到 `origin/master`）。
- **例外**：用户当次明确说「不要提交 / 不要推送」、或仅咨询无需改代码、或仓库无 `git`/无 `remote` 时，不强行推送。
- **勿提交**：`.env`、含真实密钥的文件；提交信息用简短中文或英文完整句说明本次变更。
